...
public final class Immortal implements Runnable {
  ...
  private final ImmortalManager manager;
  private final ScoreBoard scoreBoard;
  private final PauseController controller;
  private volatile boolean running = true;

  public Immortal(String name, int health, int damage, List<Immortal> population, ImmortalManager manager,
    ...
    synchronized (this) {
      synchronized (other) {
        if (this.health <= 0 || other.health <= 0)
          return;
        int damageDealt = Math.min(this.damage, other.health);
